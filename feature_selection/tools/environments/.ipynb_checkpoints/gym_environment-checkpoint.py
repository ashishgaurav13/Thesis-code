...
class HalfCheetahWithPos(HalfCheetahEnv):
    ...
    def new_reward(self, xposbefore, xposafter, action):
        reward_ctrl = -0.1 * np.square(action).sum()
        reward_dist = abs(xposafter) # /5 # max(abs(xposafter), np.log(abs(xposafter))+1e-3) 
...
class SwimmerWithPos(SwimmerEnv):
...
class WalkerWithPos(Walker2dEnv):
...
class CustomCartPoleTest(gym.Env):
    ...
    def step(self, action):
        ...
        assert self.action_space.contains(action), err_msg
        assert self.state is not None, "Call reset before using step method."
        x, x_bogus, x_dot, theta, theta_dot = self.state
        force = self.force_mag if action == 1 else -self.force_mag
        costheta = math.cos(theta)
        sintheta = math.sin(theta)
        ...
        if self.kinematics_integrator == "euler":
            x = x + self.tau * x_dot
            x_bogus = x + np.random.uniform(-self.noise, self.noise)
            x_dot = x_dot + self.tau * xacc
            theta = theta + self.tau * theta_dot
            theta_dot = theta_dot + self.tau * thetaacc
        else:  # semi-implicit euler
        ...
        self.state = (x, x_bogus, x_dot, theta, theta_dot)
    ...
    def reset(self, **kwargs):
        ...
        if self.start_pos != None:
            ...
            assert(interval[0] <= interval[1])
            self.state[0] = np.random.uniform(low=interval[0], high=interval[1])
        self.state[1] = self.state[0] + np.random.uniform(-self.noise, self.noise)
        self.steps_beyond_done = None
        return np.array(self.state, dtype=np.float32)
...
class GymEnvironment(Environment):
    ...
    def __init__(self, name, horizon=None, **kwargs):
        ...
        elif name=="CustomCartPoleTest":
            self.obj = CustomCartPoleTest(**kwargs)
        else:
